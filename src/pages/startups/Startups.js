...
function Startups() {
    ...
    const fetchData = async () => {
        try {
            setLoading(true);
            const response = await axios.get('http://localhost:3003/api/v1/fetch-startup');
    ...
    const handleGraduate = async (email) => {
        try {
            await axios.put(`http://localhost:3003/api/v1/update-status?startup_status=Graduated&official_email_address=${email}`);
    ...
    const handleDrop = async (email) => {
        try {
            await axios.put(`http://localhost:3003/api/v1/update-status?startup_status=Dropped&official_email_address=${email}`);
    ...
    const handleActive = async (email) => {
        try {
            await axios.put(`http://localhost:3003/api/v1/update-status?startup_status=Active&official_email_address=${email}`);
    ...
    const getStatusColor = (status) => {
        switch (status?.toLowerCase()) {
            case 'active': return 'bg-[#D8F3D9] text-[#45C74D]';
            case 'graduated': return 'bg-[#E8F5E8] text-[#2E7D32]';
            case 'dropped': return 'bg-[#FFEBEE] text-[#D32F2F]';
                ...
                <div className="bg-gray-100 min-h-screen">
                    <div className={`mx-10 py-5 ${showw ? "visible" : ""}`}>
                        <div className="border bg-white">
                            <div className="px-5 pt-6 text-sm text-[#808080]">Dashboard {'>'} Start-ups</div>
                                            ...
                                            {currentItems.length > 0 ? currentItems.map((startup, index) => (
                                                <div key={startup.id || index} className="border shadow-md rounded-lg p-4 bg-white">
                                                    ...
                                                    <div className="text-center font-semibold text-lg mb-1">
                                                        {startup.startup_name || startup.company_name || 'Start-up Name'}
                                                    </div>
                                                    <div className="text-center text-sm text-gray-500 mb-2">
                                                        {startup.startup_yog ? new Date(startup.startup_yog).toLocaleDateString('en-US', { month: '2-digit', year: '2-digit' }) : 'MM/YY'}
                                                    </div>
                                                    <div className="flex justify-between text-sm mt-4 text-gray-600">
                                                        <div>
                                                            <div className="font-semibold">Founder</div>
                                                            <div>{startup.founder_name || startup.founder_name}</div>
                                                        </div>
                                                        <div>
